...
def parse_args():
    ...
    parser.add_argument('-n', '--nnormalize', action='store_true', type=bool, help="Flag to let script know that it should _not_ normalize the UMCC string.  By default the UMCC is normalized.")
...
class UMCCEncoder(object):
    ...
    def __init__(self, normalize=True, contours=None):
        self.Ux = np.array([])
        self.Uy = np.array([])
        self.x  = 0
        self.y  = 0
    ...
    def doNormalize(self):
        ...
        print("kx: {}, ky: {}, K: {}".format(kxshifts,kyshifts,Kshifts))
        #Now shift right
        Ux = Ux_min
    ...
    def encode(self, contours):
        ...
        fy = 0 #Initialize sign-change flag to 1 in y-direction
        ...
        for i in range(1,len(contours)):
            cprev = contours[i-1]
            print("cprev: {}".format(cprev))
            ccurr = contours[i]
            print("ccurr: {}".format(ccurr))
            cdiff = (ccurr - cprev)[0]
            print("cdiff: {}".format(cdiff))
            ...
            print("sx: {}, fx: {}, sy: {}, fy: {}".format(sx,fx,sy,fy))
...
if __name__ == '__main__':
    ...
    umcc        = UMCCEncoder(contours=contours)
    print(umcc)
