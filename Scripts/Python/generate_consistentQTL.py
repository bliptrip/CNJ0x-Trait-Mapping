...
columns_output = ("id", "trait", "model", "chr", "position", "mean_marker_variance", "population", "method", "study", "interval_left", "interval_right", "count", "model_count", "study_count", "trait_count", "method_count", "population_count")
...
def filter_qtl(_data, *filters):
    ...
    _data = _data[list(columns_output)]
...
#Read in model config file that maps trait names to 
...
columns_highlighted = ("method", "trait", "chr", "model", "position", "nearest_marker", "qtl_lod", "marker_variance", "interval_left", "interval_right", "population","study")
#filter_small = (f.count >= 2) & (f.mean_marker_variance > 10.0) & (f.model_count >= 3)
...
qtls_cnj02 = tibble(pd.read_csv("../../Workflows/9/traits/qtl_collated.consensus.csv")) >> \
...
qtls_cnj04 = tibble(pd.read_csv("../../Workflows/10/traits/qtl_collated.consensus.csv")) >> \
                filter((f.chr2 == '') | np.isnan(f.chr2)) >> \
                rename(qtl_lod="qtl.lod",
                    marker_variance="marker.variance",
                    nearest_marker="nearest.marker",
