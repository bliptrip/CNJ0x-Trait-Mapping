...
generate_qtl_collate <- function(qtl, qtl_model, method, model, year, trait.names, trait, trait_subfolder_path, collated.df.p) {
...
collateQtlCB     <- function(trait.cfg, trait.names, traits, trait.path, funArgs) {
    ...
    for( trait in traits ) {
        ...
        generate_qtl_collate(scan.one.qtl, scan.one.md, "scanone", model, year, trait.names, trait, trait_subsubfolder_fpath, funArgs)
        ...
        generate_qtl_collate(scan.sw.qtl, scan.sw.md, "stepwiseqtl", model, year, trait.names, trait, trait_subsubfolder_fpath, funArgs)
    }
}
