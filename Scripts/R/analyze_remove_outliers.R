...
dataset.filename  <- "../../Data/phenotypic data/DerivedData/Data-combined-collated.all.csv"

#CNJ0[24] Dataframe
#NOTE: If you have problems reading the excel file, open in excel and resave.
cnjpop.df.raw  <- read.csv(dataset.filename)
colnames(cnjpop.df.raw) <- c("population", "year", "accession", "accession_name", "upright", "num_peds", "num_no_fruit", "num_berries", "num_aborted_flower", "num_aborted_berries", "total_berry_weight", "upright_length", "secondary_growth", "dry_wt_leaves", "berry_length", "berry_width", "berry_weight", "calyx_diam", "calyx_lobe_form", "calyx_lobe_size", "shape_calyx_end", "shape_stem_end", "berry_skin", "berry_shape", "num_seeds", "rebud", "notes1", "notes2", "berry_shape_redundant", "chimera_LvW", "chimera_eccentricity", "chimera_solidity", "chimera_tortuosity", "chimera_umccX", "chimera_umccLogX", "chimera_umccY", "chimera_umccLogY")
...
cleanup_numeric_helper <- function(text) {
    if( grepl("(n/a)|(n\\a)|(no tissue)|(^-$)|(_)|(rotten)", text) ) {
        text <- NA
    }
...
cnjpop.cols.include   <- c(cnjpop.cols.include.prefix, "num_peds", "num_no_fruit", "num_berries", "num_aborted_flower", "num_aborted_berries", "total_berry_weight", "upright_length", "secondary_growth", "dry_wt_leaves", "berry_length", "berry_width", "berry_weight", "num_seeds", "calyx_diam", "calyx_lobe_form", "calyx_lobe_size", "berry_skin", "rebud", "chimera_LvW", "chimera_eccentricity", "chimera_solidity", "chimera_tortuosity", "chimera_umccLogX", "chimera_umccLogY", "chimera_umccX", "chimera_umccY")
cnjpop.df <- cnjpop.df.raw %>% select(cnjpop.cols.include) #Filter out columns we care about

#Refactor categorical traits, e.g. - calyx form, calyx size, and rebud, as numeric values
cnjpop.new.df <- cnjpop.df %>%
                mutate(calyx_lobe_form = sub("^O.*", "0", calyx_lobe_form, ignore.case=TRUE, fixed=FALSE)) %>%
                mutate(calyx_lobe_form = sub("^M.*", "1", calyx_lobe_form, ignore.case=TRUE, fixed=FALSE)) %>%
                mutate(calyx_lobe_form = sub("^C.*", "2", calyx_lobe_form, ignore.case=TRUE, fixed=FALSE)) %>%
                mutate(calyx_lobe_form = as.numeric(calyx_lobe_form)) %>%
                mutate(calyx_lobe_size = sub("^L.*", "0", calyx_lobe_size, ignore.case=TRUE, fixed=FALSE)) %>%
                mutate(calyx_lobe_size = sub("^M.*", "1", calyx_lobe_size, ignore.case=TRUE, fixed=FALSE)) %>%
                mutate(calyx_lobe_size = sub("^S.*", "2", calyx_lobe_size, ignore.case=TRUE, fixed=FALSE)) %>%
                mutate(calyx_lobe_size = as.numeric(calyx_lobe_size)) %>%
                mutate(rebud = sub("^N.*", "0", rebud, ignore.case=TRUE, fixed=FALSE)) %>%
                mutate(rebud = sub("^Y.*", "1", rebud, ignore.case=TRUE, fixed=FALSE)) %>%
                mutate(rebud = as.numeric(rebud))
cnjpop.df <- cnjpop.new.df
...
#Berry skin bloom parameters -- some are marked as rotten
cnjpop.df$berry_skin = vapply(cnjpop.df$berry_skin, FUN=cleanup_numeric_helper, FUN.VALUE=as.numeric(NA))
berry_skin_na.tf = is.na(cnjpop.df$berry_skin)
saveRDSw(berry_skin_na.tf, "berry_skin_na.tf.rds", compress=TRUE)
#Calyx Diameter Params
cnjpop.df$calyx_diam = vapply(cnjpop.df$calyx_diam, FUN=cleanup_numeric_helper, FUN.VALUE=as.numeric(NA))
calyx_diam_na.tf = is.na(cnjpop.df$calyx_diam)
saveRDSw(calyx_diam_na.tf, "calyx_diam_na.tf.rds", compress=TRUE)
...
#subset.quantiles <- quantile(data[subset.ix,variable], probs=seq(0,1,1/10), na.rm = TRUE)
