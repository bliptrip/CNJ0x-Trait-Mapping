...
#Get a list of all the folders in 'folder'
folder <- paste0(workflow,"/perms")
...
for ( i in 1:length(dirs) ) {
    ...
    operms.sub <- readRDS(paste0(subdirs_fpath[[1]],'/operms.',as.character(subdirs[[1]]),'.rds'))
    operms <- operms.sub[["perms"]]
    for ( j in 2:length(subdirs) ) {
        operms.sub <- readRDS(paste0(subdirs_fpath[[j]],'/operms.',as.character(subdirs[[j]]),'.rds'))
        operms <- c(operms,operms.sub[["perms"]])
    }
    saveRDS(operms,paste0(workflow,'/traits/',dir,'/operms.rds'),compress=TRUE)
}
