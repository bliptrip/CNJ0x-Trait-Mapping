    ...
    if (mmer$description %in% query_mmers.v ) {
        ...
        for(j in 1:length(mmer$analyses)) {
            ...
            if( length(query_traits.l.idx) == 1 ) {
                ...
                cnjpop.operms.p2[[as.character(mmer$description)]][[k]] <- scantwo(cnjpop.cross, n.perm=n.perms, pheno.col=query_subtraits.v.phenos, verbose=FALSE)
...
rds.filename <- paste0("cnjpop.operms.2D.p2.",as.character(process),".rds")
