...
genome_new            <- get0("genome_new", ifnotfound="../../Data/genetic_data/RawData/GCA_000775335.2_ASM77533v2_genomic.fna")
...
generate_qtl_genomelocations <- function(method, model, trait, lg, pos, interval_left, interval_right, loopArgs) {
    map <- loopArgs$map
    src <- loopArgs$src #Source DB to pull sequences from for querying on destination blast db
    dst <- loopArgs$dst #Destination BLAST DB

    sorted_map <- map %>%
        filter(lg == LG) %>%
        mutate(distance = abs(consensus - pos),
               distance_left = abs(consensus - interval_left), %>%
               distance_right = abs(consensus - interval_right))

    left  <- (supermap.sorted.df %>% arrange(distance_left))[1]
    right <- (supermap.sorted.df %>% arrange(distance_right))[1]
    middle <- (supermap.sorted.df %>% arrange(distance))[1]
