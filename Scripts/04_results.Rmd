...
qtls.cnj02.sw.ay.ints <- read_csv(paste0(cnj02_workflow,"/traits/qtl_collated.csv")) %>%
...
qtls.cnj02.sw.ay.markv <- mean(qtls.cnj02.sw.ay.ints$marker_variance)
...
qtls.cnj04.sw.ay.ints <- read_csv(paste0(cnj04_workflow,"/traits/qtl_collated.csv")) %>%
                            filter(method=="stepwiseqtl") %>%
                            filter(model=="all-years") %>%
                            filter(!is.na(chr2) & !is.na(position2))
qtls.cnj04.sw.ay.nints <- nrow(qtls.cnj04.sw.ay.ints)
qtls.cnj04.sw.ay.ntraits <- length(unique(qtls.cnj04.sw.ay.ints$trait))
qtls.cnj04.sw.ay.markv <- mean(qtls.cnj04.sw.ay.ints$marker_variance)
...
percent, with an average model variance of `r qtls.cnj02.sw.modv` percent.  A more detailed exploration
...
CNJ02, the berry *chimera length versus width* and *chimera eccentricity* represented good proxies.  A collocated QTL on 
...
The `stepwiseqtl` method found `r qtls.cnj02.sw.ay.nints` significant pairwise QTL:QTL interactions across `r qtls.cnj02.sw.ay.ntraits` 
for the *All Years* model, with an average variance per QTL pair of `r qtls.cnj02.sw.ay.markv`.  Table \@ref(tab:qtl-table-cnj02-sw-ints) 
summarizes the top pairwise QTL interactions
...
generateReducedTable(effs.ints.all.tb %>% 
                    ...
                    slice_head() %>%
                    ungroup(),
                    caption="CNJ02 Population Top Interaction QTLs for *All Years* model, QTLs fit using *stepwiseqtl()* function.  Only models with heritability > 0.5, QTLs with interaction percent variance explained greater than 5.0%, and models with significant genotypic effects (*p < 0.05*) are displayed.")
```
...
noteable for having the highest number of large-effect yield QTLs.  On linkage group 10, largest *berry length* and largest
...
related to large QTLs closely linked on linkage group 7 (position 38cM and 33cM, respectively). 
...
positive *Tacy* effect and substantial negative *plot mean fruit mass* effect  The largest QTL for *titratable acidity*, 
found on linkage group 2 at 50cM, appear to be closely linked with other traits' major QTLs.   A closely
linked pair of QTL on linkage group 3 between *plot mean fruit mass* and *titratable acidity* could explain the
modest positive correlation between these two traits (0.40).
...
generateReducedTable(effs.ints.all.tb %>% 
                    ...
                    slice_head() %>%
                    ungroup(),
                    caption="CNJ04 Population Top Interaction QTL for *All Years* model, QTLs fit using *stepwiseqtl()* function.  Only models with heritability > 0.5 and with significant genotypic effects (*p < 0.05*) are displayed.")
```
