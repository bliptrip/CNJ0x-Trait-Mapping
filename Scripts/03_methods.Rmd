...
```{r trait-tab, echo=FALSE, message=FALSE, include=TRUE, results='asis'}
library(tidyverse)
library(kableExtra)
...
trait_tab_cat <- trait_tab %>% 
    ...
    rename('Category[note]'=Category,'Abbreviation[note]'=Abbreviation)
kable(trait_tab_cat, 
      "html",  
      booktabs=TRUE, 
      escape = FALSE,
    ...
    row_spec(row=which(trait_tab_cat["Category[note]"] != ""), extra_css = "border-top: 1px solid #ddd") %>%
...
```{r cat-trait-tab, echo=FALSE, message=FALSE, include=TRUE, results='asis'}
...
kable(cat_trait_tab %>% dplyr::select(Trait,'Categorical Value','Numeric Value'), 
        "html",  
        booktabs=TRUE, 
        escape = FALSE,
        align="llr",
        caption="List of categorical traits and their mapped numeric values.  Generally, numeric values were chosen such that larger values represented more desirable breeding attributes.") %>%
        column_spec(1, bold=TRUE) %>%
        row_spec(row=which(cat_trait_tab$Trait != ""), extra_css = "border-top: 1px solid #ddd") %>%
        kable_paper("striped", full_width=FALSE)
```
