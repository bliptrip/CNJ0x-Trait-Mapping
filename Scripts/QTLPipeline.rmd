...
cnj02.merged.standardized.tb <- cbind(cnj02.merged.mmeans.tb %>% dplyr::select(all_of(id_columns)), apply(cnj02.merged.mmeans.tb %>% dplyr::select(!id_columns), 2, standardize))
cnj02.merged.mmeans.tb[is.na(cnj02.merged.standardized.tb)] <- NA
cnj04.merged.standardized.tb <- cbind(cnj04.merged.mmeans.tb %>% dplyr::select(all_of(id_columns)), apply(cnj04.merged.mmeans.tb %>% dplyr::select(!id_columns), 2, standardize))
cnj04.merged.mmeans.tb[is.na(cnj04.merged.standardized.tb)] <- NA
...
for( i in 1:length(traits) ) {
  ...
  if( trait %in% trait_colnames_cnj02 ) {
    ...
    cnj02.merged.standardized.tb <- cnj02.merged.standardized.tb %>% rename_with(~ gsub(trait, trait_label, .x, fixed=TRUE), .cols=trait)
  }
  ...
  if( trait %in% trait_colnames_cnj04 ) {
    ...
    cnj04.merged.standardized.tb <- cnj04.merged.standardized.tb %>% rename_with(~ gsub(trait, trait_label, .x, fixed=TRUE), .cols=trait)
  }
  if( trait %in% trait_colnames_cnj02.mmeans ) {
...
cnj02.merged.mmeans.tb <- cnj02.merged.mmeans.tb %>%
                            mutate(year = gsub("20([0-9]{2})", "\\1", year)) #Abbreviate the years for compact display of labels
cnj02.merged.standardized.tb <- cnj02.merged.standardized.tb %>%
                            mutate(year = gsub("20([0-9]{2})", "\\1", year)) #Abbreviate the years for compact display of labels
cnj04.merged.mmeans.tb <- cnj04.merged.mmeans.tb %>%
                            mutate(year = gsub("20([0-9]{2})", "\\1", year)) #Abbreviate the years for compact display of labels
cnj04.merged.standardized.tb <- cnj04.merged.standardized.tb %>%
                            mutate(year = gsub("20([0-9]{2})", "\\1", year)) #Abbreviate the years for compact display of labels
```
...
d_cnj02 <- dist(cnj02.merged.standardized.tb %>% select(BBI_UTBM:PAC))
dissplot(d_cnj02)
```
...
gencorrplot <- function(data.tb, data_is_cor=FALSE, filename=NULL, min_cor=0.0, width=1280, height=1024, seriate_method=NULL, method="square", type="lower", cl.pos='r', rect.spec=NULL, invert=FALSE, ...) {
    if( !is.null(filename) ) {
...
gencorrplot.mixed <- function(data.tb, data_is_cor=FALSE, filename=NULL, min_cor=0.0, width=1280, height=1024, seriate_method=NULL, lower="square", upper="number", cl.pos='r', na.label = " ", invert=FALSE, rect.spec=NULL, ...) {
    if( !is.null(filename) ) {
        png(filename=filename, bg="transparent", width=width, height=height, units='px')
    }
    col.gradient <- col1(100)
...
setwd(rRoot)
p.cnj02 = gencorrplot.mixed(cnj02.merged.means.tb %>% select(!c(accession_name, row, column)), width=2048, height=2048, tl.cex=0.75, cl.cex=0.75, number.cex=0.75,seriate_method="TSP")
...
setwd(rRoot)
p.cnj02.inv = gencorrplot.mixed(cnj02.merged.means.tb %>% select(!c(accession_name, row, column)), width=2048, height=2048, tl.cex=0.75, cl.cex=0.75, number.cex=0.75,seriate_method="TSP", invert=TRUE)
...
setwd(rRoot)
p.cnj04 = gencorrplot.mixed(cnj04.merged.means.tb %>% select(!c(accession_name, row, column)), width=2048, height=2048, tl.cex=0.75, cl.cex=0.75, number.cex=0.75, seriate_method="TSP")
...
setwd(rRoot)
p.cnj04.inv = gencorrplot.mixed(cnj04.merged.means.tb %>% select(!c(accession_name, row, column)), width=2048, height=2048, tl.cex=0.75, cl.cex=0.75, number.cex=0.75, seriate_method="TSP",invert=TRUE)
...
setwd(rRoot)
p.cnj02.inv.refined = gencorrplot.mixed(cnj02.merged.means.tb %>% select(!c(accession_name, row, column)) %>% select(c(PAC,TY,UCLP,UCLS,ULvW,UKTO,Brix,MFM,UMFM)), width=2048, height=2048, tl.cex=0.75, cl.cex=0.75, number.cex=0.75,seriate_method="TSP", invert=TRUE)
...
setwd(rRoot)
p.cnj04.inv.refined = gencorrplot.mixed(cnj04.merged.means.tb %>% select(!c(accession_name, row, column)) %>% select(c(URB,USL,UNS,MFM,BBITBM,UBW,UCLP,UCLS,UNB,PFR,PAC,TY,UBL,Brix,UBM)), width=2048, height=2048, tl.cex=0.75, cl.cex=0.75, number.cex=0.75,seriate_method="TSP", invert=TRUE)
...
gencorrplot.mixed(p.cnj02.refined, data_is_cor=TRUE, filename=paste0(workflow,"/traits/plots/corrplot.cnj02.refined.png"), width=2048, height=2048, tl.cex=1, cl.cex=1, number.cex=1.5)
p.cnj02.inv.refined = readRDS(paste0(workflow,"/traits/plots/corrplot.cnj02.inv.refined.rds"))
gencorrplot.mixed(p.cnj02.inv.refined, data_is_cor=TRUE, filename=paste0(workflow,"/traits/plots/corrplot.cnj02.inv.refined.png"), width=2048, height=2048, tl.cex=1, cl.cex=1, number.cex=1.5, invert=TRUE)
p.cnj04.refined = readRDS(paste0(workflow,"/traits/plots/corrplot.cnj04.refined.rds"))
gencorrplot.mixed(p.cnj04.refined, data_is_cor=TRUE, filename=paste0(workflow,"/traits/plots/corrplot.cnj04.refined.png"), width=2048, height=2048, tl.cex=1, cl.cex=1, number.cex=1.5)
p.cnj04.inv.refined = readRDS(paste0(workflow,"/traits/plots/corrplot.cnj04.inv.refined.rds"))
gencorrplot.mixed(p.cnj04.inv.refined, data_is_cor=TRUE, filename=paste0(workflow,"/traits/plots/corrplot.cnj04.inv.refined.png"), width=2048, height=2048, tl.cex=1, cl.cex=1, number.cex=1.5, invert=TRUE)
```
