...
png(filename=paste0(workflow,"/traits/plots/corrplot.cnj02.flat.png"), bg="white", width=2048, height=1024, units='px')
p <- ggplot(pf.cnj02.extremes, aes(x=row_column,y=cor)) + 
		...
		scale_fill_distiller(
				...
				palette="RdBu",
				direction=-1) +
        geom_text(aes(label=sprintf("%0.2f", round(cor, digits = 2)),fontface="bold"),size=7,nudge_y=ifelse(pf.cnj02.extremes$cor < 0, -0.05, 0.05)) +
        ylim(-1.2,1.2) +
        theme_minimal() +
        theme(axis.title = element_text(face="bold",size=14),
              axis.text.x  = element_text(face="bold",size=16,angle=75),
...
pf.cnj04.extremes <- rbind(pf.cnj04 %>% head(top_rank), pf.cnj04 %>% tail(top_rank)) %>%
                        mutate(row_column=paste0(row," × ",column)) %>%
                        mutate(row_column=factor(row_column, levels=row_column, ordered=TRUE))
png(filename=paste0(workflow,"/traits/plots/corrplot.cnj04.flat.png"), bg="white", width=2048, height=1024, units='px')
p <- ggplot(pf.cnj04.extremes, aes(x=row_column,y=cor)) + 
		...
		scale_fill_distiller(
				...
				palette="RdBu",
				direction=-1) +
        geom_text(aes(label=sprintf("%0.2f", round(cor, digits = 2)),fontface="bold"),size=7,nudge_y=ifelse(pf.cnj04.extremes$cor < 0, -0.05, 0.05)) +
        ylim(-1.2,1.2) +
        theme_minimal() +
        theme(axis.title = element_text(face="bold",size=14),
              axis.text.x  = element_text(face="bold",size=16,angle=75),
